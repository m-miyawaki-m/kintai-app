...
**主任（supervisor）の場合:**
- 自分の勤怠記録の下に「配下メンバーの勤怠状況」セクションを表示
- 配下メンバー（subordinates）の勤怠をカード一覧で表示
- リアルタイムで更新
...
interface User {
  ...
  role: 'user' | 'supervisor' | 'admin' // ユーザー種別
  subordinates?: string[] // 配下メンバーのUID配列（主任のみ）
...
| supervisor@example.com | password | 鈴木 一郎 | supervisor |
| user01@example.com | password | 山田 太郎 | user |
| user02@example.com | password | 佐藤 花子 | user |
| user03@example.com | password | 田中 次郎 | user |
| user04@example.com | password | 高橋 美咲 | user |
| user05@example.com | password | 伊藤 健太 | user |

※ 主任（鈴木 一郎）には user01〜user05 が配下メンバーとして設定される
...
src/
├── stores/__tests__/
│   └── auth.test.ts          # 認証ストアのテスト
├── components/__tests__/
│   └── AttendanceRecordCard.test.ts  # 勤怠カードのテスト
```
...
| ドキュメント | 内容 |
|-------------|------|
| `docs/testing/TEST_CASES.md` | テストケース一覧、実施記録 |
| `docs/BUG_TRACKING.md` | バグ・修正管理表 |
...
| 2025-02-05 | 主任ロール（supervisor）追加、配下メンバー勤怠表示機能 |
| 2025-02-05 | テスト環境構築（Vitest + Vue Test Utils） |
| 2025-02-05 | テストケース管理、バグ管理表作成 |
